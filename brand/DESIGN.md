...
Das CI hat zwei Welten. Sie sind nie gemischt. Umsetzung als Light/Dark-Toggle:

| Welt | Theme | Zielgruppe | Primär-Touchpoints |
|---|---|---|---|
| Education | **Light** (Rosa/Rot) | Studierende, Hochschulen | tools-app, Circle, Sales-Decks Hochschulen |
| Business | **Dark** (Blau/Neon) | Partner, Corporate, Investoren | Website Landing, Pitch-Decks |

**Toggle-Verhalten:**
- Default = System-Präferenz (`prefers-color-scheme`)
- User-Toggle überschreibt, gespeichert in LocalStorage
- **Kein Flächen-Flood**: Rosa/Blau sind Akzente (Header-Band, Buttons, Hover-Glow), nicht Seiten-BG. Body-BG bleibt Neutral (`#F6F6F6` Light / `#141414` Dark).

**Mail-Strategie:**
- Alle Mails (Auth + Marketing) adaptiv via `@media (prefers-color-scheme: dark)`
- Kein Segment-basiertes Theming — ein Template pro Mail-Typ
...
| Name | Hex | Rolle |
|---|---|---|
| Rosa | `#FC78FE` | Education-Header-BG (Light) |
| Rot | `#F5133B` | Education-Primary-Action (Light) |
| Blau | `#3A3AFF` | Business-Header-BG (Dark) |
| Neon-Grün | `#CEFF32` | Business-Primary-Action (Dark) |
...
**Kurz:**
- Ansprache: **Du in Education + Business** (E1=a, überall Du)
...
| `logo-wide-pink-red.svg` | Rosa "Generation" + Rot "AI" | Education-Deck-Cover |
| `logo-wide-red-on-pink.svg` | Rot auf Rosa-BG (Hero) | Social / OG Education |
| `logo-wide-neon.svg` | Neon-Grün uni | Dark-Theme Header + Mail Dark |
| `logo-wide-blue.svg` | Blau uni | Alternativer Akzent Dark |
| `logo-wide-blue-neon.svg` | Blau "Generation" + Neon "AI" | Business-Deck-Cover |
| `logo-wide-neon-on-blue.svg` | Neon auf Blau-BG (Hero) | Social / OG Business |
...
| Social Profil Business | `logo-wide-neon-on-blue.svg` |
| Social Profil Education | `logo-wide-red-on-pink.svg` |
| OG-Image Default | `logo-wide-neon-on-blue.svg` |
| Pitch-Deck (Business) | `logo-wide-blue-neon.svg` |
| Sales-Deck (Hochschulen) | `logo-wide-pink-red.svg` |
| Print (warm/Papier) | `logo-wide-sand.svg` |
| Favicon | **TBD — simpler Symbol-Style, kommt separat** |
...
- **Square-Logo-Varianten** (für App-Icons, Avatare) — nicht in aktuellen Assets, später vom Designer ziehen
- **Weiß-Outline-Variante** (laut CI-PDF existiert, nicht in Dateien) — nachfordern
- **Favicon** — simpler Symbol-Cut, Briefing offen
