...
export function LabeledNodes({
  ...
  useEffect(() => {
    ...
    // Auto-wander cluster (mobile only, when no cursor hover). Cluster drifts to
    // a new random spot every 3.5-5s and lerps smoothly between positions —
    // shows the label/cluster motif passively on devices without hover.
    const auto = {
      x: -9999,
      y: -9999,
      targetX: 0,
    ...
    const buildNodes = () => {
      ...
      // Mobile deutlich luftiger: weniger Nodes + höherer Divisor.
    ...
    const frame = (t: number) => {
      ...
      // 1. Move nodes (skip under reduced-motion)
      if (!reduced) {
      ...
      // 2a. Auto-wander (mobile, no hover). Every ~3.5-5s pick a new random
      // target (avoid the dead-center text area by biasing toward the margins),
      // then lerp the auto-cluster position smoothly toward it.
      const isMobile = state.w < 768
      const autoActive = isMobile && !mouse.active && !reduced
      if (autoActive) {
        if (!auto.inited || (t || 0) >= auto.nextRetargetMs) {
          // Bias target toward corners/edges so the cluster doesn't camp
          // behind the centered text. Pick from 4 edge quadrants randomly.
          const edgeT = 0.18 + Math.random() * 0.22 // 0.18..0.40 from edge
        ...
        // Smooth lerp toward target (~60fps → ~3s to close most of gap)
        auto.x += (auto.targetX - auto.x) * 0.018
        auto.y += (auto.targetY - auto.y) * 0.018
      }

      // Safe-Zone (Content-Area): Nodes im mittleren Rechteck werden gefadet,
      // Cluster-Detection überspringt sie. Ellipsen-Distanz zum Zentrum.
      //   factor=0 im Zentrum, =1 am Ellipsen-Rand und darüber hinaus.
      // Größen (mobile schmaler weil Content schmaler ist):
      ...
      // 2b. Cluster detection (cursor OR auto-wander, min 3 nodes).
      // Safe-Zone-Nodes (factor < 0.35) werden ausgeschlossen → Cluster bilden
      // sich nur außerhalb des Content-Bereichs.
      ...
      // 3. Dynamic label assignment (stable: labels persist while node stays active)
      ...
      // Nodes no longer in cluster: preserve lastLabel, clear live label
      for (let i = 0; i < nodes.length; i++) {
        const n = nodes[i]!
      ...
      // 4. Smooth activation lerp per node
      for (let i = 0; i < nodes.length; i++) {
        const target = clusterIdx.has(i) ? 1 : 0
        nodes[i]!.act += (target - nodes[i]!.act) * 0.08
      }

      // 5. Lines (base constellation; neon between active cluster pair).
      // Safe-Zone: Linien werden an Endpunkten gefadet — Linien durch den
      // Content-Bereich verschwinden in der Mitte.
      ctx.lineWidth = 1
      for (let i = 0; i < nodes.length; i++) {
        const a = nodes[i]!
        ...
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (d2 < PROX * PROX) {
            ...
            // Safe-Zone fade: min der beiden Endpunkt-factors
      ...
      // 6. Directional flow particles between active cluster members
      ...
      // 7. Nodes + labels (Safe-Zone dimmt beides)
